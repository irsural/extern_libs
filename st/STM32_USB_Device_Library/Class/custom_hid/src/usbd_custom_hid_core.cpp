...
#ifdef NOP
__ALIGN_BEGIN static uint8_t CustomHID_ReportDescriptor[CUSTOM_HID_SIZ_REPORT_DESC_1] __ALIGN_END = { // USB report descriptor         // ?????????? ????????? ????????? ?????? ?????? ??? ??????
...
#ifdef NOP
__ALIGN_BEGIN static uint8_t CustomHID_ReportDescriptor[CUSTOM_HID_SIZ_REPORT_DESC_2] __ALIGN_END = { // USB report descriptor         // ?????????? ????????? ????????? ?????? ?????? ??? ??????
    0x06, 0xff, 0x00,                       /* USAGE_PAGE (Vendor Page: 0xFF00) */
    0x09, 0x01,                             // USAGE (Vendor Usage 1)
    0xa1, 0x01,                             // COLLECTION (Application)
    0x15, 0x00,                             //    LOGICAL_MINIMUM (0)        // min. ???????? ??? ??????
...
#ifdef NOP
__ALIGN_BEGIN static uint8_t CustomHID_ReportDescriptor[CUSTOM_HID_SIZ_REPORT_DESC_1] __ALIGN_END = { // USB report descriptor         // ?????????? ????????? ????????? ?????? ?????? ??? ??????
    0x06, 0xff, 0x00,                       // USAGE_PAGE (Generic Desktop)
    0x09, 0x01,                             // USAGE (Vendor Usage 1)
    0xa1, 0x01,                             // COLLECTION (Application)
    0x15, 0x00,                             //    LOGICAL_MINIMUM (0)        // min. ???????? ??? ??????
    0x26, 0xff, 0x00,                       //    LOGICAL_MAXIMUM (255)      // max. ???????? ??? ??????, 255 ??? ?? ????????, ? ????? ????????? ? 1 ????
...
#endif // NOP
#ifdef NOP
__ALIGN_BEGIN static uint8_t CustomHID_ReportDescriptor[HID_MOUSE_REPORT_DESC_SIZE] __ALIGN_END =
{
  0x05,   0x01,
  0x09,   0x02,
  0xA1,   0x01,
...
#endif // NOP
#ifdef NOP
__ALIGN_BEGIN static uint8_t CustomHID_ReportDescriptor[CUSTOM_HID_SIZ_REPORT_DESC] __ALIGN_END =
{
  0x06, 0xFF, 0x00,      /* USAGE_PAGE (Vendor Page: 0xFF00) */
...
static uint8_t  USBD_HID_DataOut (void *pdev, uint8_t epnum)
{
  ...
  if (epnum == HID_OUT_EP)
  {
    ...
    if (((USB_OTG_CORE_HANDLE*)pdev)->dev.device_status == USB_OTG_CONFIGURED )
    {
      ...
      //data_out_read_count_test += USB_RecData_Cnt - 3;
      //data_out_read_all_count_test += USB_RecData_Cnt - 3;
      data_out_read_count_test += *reinterpret_cast<irs_u16*>(USB_HID_Rx_Buffer + 1);
      data_out_read_all_count_test += *reinterpret_cast<irs_u16*>(USB_HID_Rx_Buffer + 1);
      //static irs::loop_timer_t timer(irs::make_cnt_s(1));
      //static irs::measure_time_t time;
      //if (timer.check()) {
      //USB_HID_Rx_Buffer[USB_RecData_Cnt] = '\0';
        //IRS_LIB_DBG_MSG("read_byte_count = " << count << " read_speed = " <<
          //count/time.get() );
      //}
